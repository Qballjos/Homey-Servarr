...
class ServarrAPI {
  ...
  constructor(config) {
    ...
    // Prefer explicit port in the URL, else provided config, else default
    this.port = parseInt(parsed.port, 10) || this.port;
    this.basePath = parsed.pathname.replace(/\/$/, '');
    if (!this.port) {
      // Fallback defaults per app (Radarr/Sonarr/Lidarr share their default)
      this.port = 7878; // Radarr default; Sonarr/Lidarr override at call site
    }

    const appLower = (this.appName || '').toLowerCase();
    // Lidarr uses API v1; Radarr/Sonarr use v3
    this.apiVersion = config.apiVersion || (appLower === 'lidarr' ? 'v1' : 'v3');
  }
  ...
  async _request(endpoint, method = 'GET', body = null, attempt = 0) {
    return new Promise((resolve, reject) => {
      const url = `${this.basePath}/api/${this.apiVersion}${endpoint}`;
      const options = {
  ...
  async searchByTitle(title) {
    try {
      ...
      const allItems = await this._request(endpoint);
      const items = Array.isArray(allItems) ? allItems : (allItems.records || []);
      const searchTerm = title.toLowerCase();
      return items.filter(item => {
        const itemTitle = (item.title || item.series?.title || item.artistName || '').toLowerCase();
